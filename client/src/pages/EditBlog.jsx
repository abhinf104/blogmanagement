...
const EditBlog = () => {
  ...
  useEffect(() => {
    if (currentBlog) {
      ...
      if (
        user &&
        user.userId !== currentBlog.author?._id &&
        user.role !== "admin"
      ) {
        navigate(`/blog/${id}`);
        return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Dispatch update blog action
      await dispatch(updateBlog({ id, blogData })).unwrap();

      setSuccessMessage("Post updated successfully!");

      // Redirect to the blog after a delay
      setTimeout(() => {
        navigate(`/blog/${id}`);
      }, 1500);
    } catch (err) {
  ...
  if (error && !currentBlog) {
    return (
      <div className="error-container">
        ...
        <p>{error}</p>
        <button onClick={() => navigate("/Blogs")} className="btn-secondary">
          Back to blog
        </button>
      </div>
    );
